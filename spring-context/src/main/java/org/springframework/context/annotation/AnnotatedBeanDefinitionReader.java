...
public class AnnotatedBeanDefinitionReader {
	...
	public AnnotatedBeanDefinitionReader(BeanDefinitionRegistry registry, Environment environment) {
		...
		// 创建一个ConditionEvaluator对象, 按照一定的条件进行判断，满足条件给容器注册bean。
	...
	<T> void doRegisterBean(Class<T> annotatedClass, @Nullable Supplier<T> instanceSupplier, @Nullable String name,
		...
		// AnnotatedGenericBeanDefinition继承自 BeanDefinition 作用是定义一个bean的数据结构，
		...
		// 作用域写回BeanDefinition数据结构, abd中默认的情况下为空，将默认值singleton重新赋值到abd
		...
		// 根据beanName和bean定义信息封装一个beanhold,heanhold其实就是一个 beanName 和BeanDefinition的映射
