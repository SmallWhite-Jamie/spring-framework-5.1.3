...
/**
 * <p>整个容器的核心处理类，是真正 的Spring容器的执行者，在内部使用了模板方法，实现了高复用、高扩展</p>
 * <p>实现了Spring的启动、停止、刷新、事件推送、BeanFactory方法的 默认实现及虚拟机回调的注册等。</p>
...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		// 使用同步锁，防止启动、关闭及注册回调函数的重复调用，保证上下文对象的状态
		synchronized (this.startupShutdownMonitor) {
			// 1、提前准备启动参数
			// Prepare this context for refreshing.
			prepareRefresh();

			// 2、调用子类的创建工厂方法和刷新工厂的方法,例如 GenericApplicationContext 初始化时已经创建：new DefaultListableBeanFactory()
			...
			try {
				// 4、调用 BeanFactory 实例化后置处理器，空方法，用于在容器的子类中扩展
				...
				// 此方法执行后 spring中的扫描注册的业务bean以 BeanDefinitions 的形式存在
				invokeBeanFactoryPostProcessors(beanFactory);

				// *6*、注册BeanPostProcessor 后置处理器，用于拦截bean创建过程
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 设置忽略的自动装配接口，表示这些接口的实现类不允许通过接口自动注入？不理解
...
//		beanFactory.ignoreDependencyInterface(TestServiceI.class);
		...
		// 将早期后置处理器注册为application监听器，用于检测内部bean。
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 初始化所有实现 LoadTimeWeaverAware 接口的类, 由Spring动态做变换
	...
	/**
	 * 判断当前spring容器对应的BeanFactory是否可用
