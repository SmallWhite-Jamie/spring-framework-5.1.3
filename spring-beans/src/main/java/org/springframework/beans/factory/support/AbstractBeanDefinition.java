...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * Bean的 作用范围，默认为空，即单实例
	 */
	@Nullable
	private String scope = SCOPE_DEFAULT;

	/**
	 * 设置抽象标志，用于最Bean描述信息的比较
	 */
	...
	/**
	 * 依赖检查，当处理属性注入时 是否启动检查
	 */
	private int dependencyCheck = DEPENDENCY_CHECK_NONE;

	/**
	 * 当前Bean依赖的beans的名称，当获取bean时注册并实例化这些bean
	 */
	@Nullable
	private String[] dependsOn;

	/**
	 * 自动注入候选Bean，也就是说如果相同bean有多个，则优先使用当前bean值为true的Bean
	 */
	...
	/**
	 * 如果bean class的范围修饰符不是public，并且这个值设置为false，则在创建实例时检查性报错
	 */
	private boolean nonPublicAccessAllowed = true;

	/**
	 * 构造函数的参数可能有多个，所以在设置这个参数时，构造函数解析时使用宽松模式
	 */
	...
	/**
	 * Bean的三种角色：
	 *    OLE_APPLICATION = 0; 全局角色，面向spring用户；
	 *    ROLE_SUPPORT = 1;
	 *    OLE_INFRASTRUCTURE = 2;spring内部使用
	 */
